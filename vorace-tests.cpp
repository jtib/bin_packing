    ...
    for (int i = 1; i < argc; ++i) {
        if (std::string(argv[i]) == "-f") {
            if (i + 1 < argc) {  
                ...
                //Calcul du volume occupe
                int v_occupe_total=0;
                for(int i=0;i<m;i++)
                {
                    v_occupe_total += boites[i];
                }

                //Calcul du volume restant
                int v_restant=volumeTotalBoites-v_occupe_total;
                
                //Affichage des infos utiles
                //Temps pour l'exemplaire, nb d'items, nb de boites, volume inutilise
                cout << microseconds.count() << ", " << n << ", " << m << ", " <<  v_restant << endl;

                //cout << volumeTotalBoites << endl;
